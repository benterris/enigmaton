...
#python run.py <c|d> <key> [message]
#
#<c|d> : c to crypt, d t decrypt
#<key> : the key to be used to the encryption
#[message] (optionnal) : the message to encrypt : if not specified, the message used will be the one from the file input.txt, and the crypted result will be written in output.txt
...
if(len(sys.argv) == 3):
    ...
    key = sys.argv[2]
    message = readFromInput()
    if(sys.argv[1] == 'c'): #if we want to crypt
        crypt = en.code(message, key, rotors)
        writeToOutput(crypt)
        print('Message crypted in output file')
    elif(sys.argv[1] == 'd'): #if we want to decrypt
        decrypt = en.decode(message, key, rotors)
        writeToOutput(decrypt)
        print('Message decrypted in output file')
elif(len(sys.argv) == 4):
